    ...
    internal class DataPCScript : NetworkBehaviour
    {
        ...
        void ExitGameClientRpc(NetworkBehaviourReference netRef, bool succeeded)
        {
            if ((IsHost || IsServer) && succeeded)
            {
                ...
                go.GetComponent<ScrapValueSyncer>().SetScrapValue(UpgradeBus.Instance.PluginConfiguration.ContractsConfiguration.DataConfiguration.RewardValue.Value + (int)(TimeOfDay.Instance.profitQuota * Mathf.Clamp(UpgradeBus.Instance.PluginConfiguration.ContractsConfiguration.RewardQuotaMultiplier.Value / 100f, 0f, 1f)));
